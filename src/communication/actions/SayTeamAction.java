...
public class SayTeamAction extends StandardAction {

	public SayTeamAction(byte type, byte priorite, int key, String address, int port, byte[] objet) {
	...
	public void clientAction(Client client, CommClient commClient) {
		try { 
			...
			ccb.addText(ccb.textChatEquipe, p.getNom(), split[1]);
	...
	public void serveurAction(Serveur serveur, CommServeur commServeur) {
		try {
			...
			commServeur.notifierMessageEquipe(idperso, text);
		} catch (Exception ex) {
			ex.printStackTrace();	
