...
class GestTransfertImage extends TransferHandler {
	
	private static final long serialVersionUID = 1L;
	
	DataFlavor pictureFlavor = DataFlavor.imageFlavor;
    ImageEmplacement sourcePic;
    boolean shouldRemove;

    public boolean importData(JComponent c, Transferable t) {
        Image image;
        if (canImport(c, t.getTransferDataFlavors())) {
            ImageEmplacement pic = (ImageEmplacement)c;
    ...
    class PictureTransferable implements Transferable {
        ...
        PictureTransferable(ImageEmplacement pic) {
            image = pic.image;
        }
